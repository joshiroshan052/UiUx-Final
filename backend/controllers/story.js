...
exports.addSeen = async (req, res) => {
  try {
    ...
    // Check if the story owner is the same as the current user
    if (story.owner.equals(req.user._id)) {
      return res.send({ success: false, message: "Unsupported" });
    }

    // Check if the user has already seen the story
    if (story.seen.includes(req.user._id)) {
      return res.send({ success: false, message: "Unsupported" });
    }

    // Update the story to include the current user in the seen array
...
exports.homeStory = async (req, res) => {
  try {
    ...
    if (userStories.length != 0) allStories.push(userStories);

    // Add stories from followings
    await Promise.all(
      user.followings.map(async (item) => {
        ...
        if (stories.length != 0) allStories.push(stories);
      })
    );
